...
var openid = {
	...
	img_path: '/css/openid-00000001.png',
	...
	init: function(input_id) {
		...
		if (providers_small) {
			...
			for (id in providers_small) {
				openid_btns.append(this
						.getBoxHTML(providers_small[id], 'small'));
			}
		}
	...
	signin: function(box_id, onload) {
		...
		if (provider['label']) {
			...
			if (provider['oauth_version']) {
				this.setOAuthInfo(provider['oauth_version'],
						provider['oauth_server']);
			} else {
...
(function () {
	...
	var createCharacterCountOutfits = [      3,      65,      4,       5,     28,      9,    57,     19,        0 ];
	...
	$("#mousefloatingimageontopofboxes").mouseleave(function() {
		if (audio !== null) {
			audio.pause();
			audio.currentTime = 0;
		}
